...
export default class ScheduleState extends Phaser.State {
    * gen() {
        ...
        this.game.displayDialogLine('Ви', 'Ніколи ще Ви не чекали першого вересня з таким нетерпінням. І нарешті, залишивши позаду приймальну комісію, Ви стали гордим володарем студентського квитка та поки що чистої заліковки', () => this.next());
        yield;

        //пред. выбор
        if (PostIntro.GirlOrMan == "Girl"){
            this.game.displayDialogLine('Ви', 'Атмосфера актового залу, до якого Вас запросили, сповнювала душу почуттям тріумфу. Ось на сцену підійнявся чоловік, якого Ви злякалися на Дні відкритих дверей. Хто б міг подумати, що він - завідувач кафедри Комп\'ютерних наук. Не дивно, що в залі одразу настала абсолютна тиша', () => this.next());
        }
        else {
