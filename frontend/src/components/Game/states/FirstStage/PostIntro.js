...
export default class IntroState extends Phaser.State {
    * gen() {
        ...
        this.game.displayDialogLine('Ви', '*Може порадитися з нею? Чи звернутися до чоловіка?*', () => this.next());
        yield;
        ...
        if (this.firstAnswer == 'Girl'){
            this.game.displayDialogLine('Ви', 'Ну ні, чоловік більш лякає Вас, ніж привертає. Треба бути зовсім безстрашним, щоб тільки заговорити з ним. "Не сьогодні", - вирішуєте Ви і прямуєте до дівчини', () => this.next());
            yield;
            ...
            this.game.displayDialogLine('Ви', 'Посмішка вийшла натягнутою - питання хвилювало Вас і остаточної відповіді не знайшлось', () => this.next());
            yield;
            ...
            if (this.secondAnswer == "Girl1"){
                this.game.displayDialogLine('Ви', 'Більше не вагаючись, Ви розповіли їй про Ваші сумніви у власних здібностях до лякаючи складного фаху ІТ-спеціаліста. Дослухавши до кінця, вона ледь стримувала сміх, міцно стиснувши губи', () => this.next());
            }

            //Выбор: "Не открываться"
            else { 
                this.game.displayDialogLine('Ви', '*Ви стримано знизали плечима, на що дівчина здивовано підняла брови але тактовно змовчала*', () => this.next());
                yield;
                this.game.displayDialogLine('Ви', 'Відчув себе ніяково, Ви все ж вирішили сказати, що вас турбує', () => this.next());
            }
            yield;

            this.game.displayDialogLine('Надія', 'Комп’ютерні науки? Кльово! Це ж моя кафедра, я - студентка четвертого курсу. Пробач, але які ж дивні міфи вигадали о професії! Не сперечаюсь, щоб досягти успіху, треба невтомно працювати, постійно покращувати soft skills, але як інакше? Без зусиль лише щасливці в лотереї виграють', () => this.next());
            yield;

            this.game.displayDialogLine('Ви', 'Ви важко зітхнули і дослухалися до її слів. Соромно зізнатись, але донедавна Ви були впевнені, що у програмуванні жінкам не місце… Ох уж ці стереотипи! Крім того, Надія не виглядала ні дивною, ні асоціальною, ні занедбаною. Ваші сумніви поряд з нею здавалися надуманими та сміховинними', () => this.next());
            yield;

            this.game.displayDialogLine('Надія', 'Тільки поміркуй, зараз інформаційні технології вкоренилися майже у всіх сферах життя. Комунікації, медицина, освіта, промисловість, фінанси... Ти маєш змогу реалізувати себе в будь-якій галузі. Мене шалено приваблює саме ця можливість вибору, перехрестя безлічі доріг', () => this.next());
            yield;

            this.game.displayDialogLine('Ви', 'Вам стало цікаво, якою з доріг крокує вона у цю мить, і Ви точно не розраховували почути "інженер Machine learning". Зате дізналися, що за цією страшною назвою, ховаються ті, хто навчають машинні нейронні мережі виявляти закономірності на підставі аналізу відомостей. Тобто зрозуміли Ви вельми наближено але перейнялись', () => this.next());
            yield;

            this.game.displayDialogLine('Надія', 'Тож як розвіяти твої сумніви? Мені подобається думка, що я на спеціальності супергероїв. Деякі з нас рятують мережу від вірусів, хтось - створює віртуальні всесвіти, інші - силою думки перетворюють обчислювальні машини у розумних істот, що здатні швидко виконувати важкі для людини речі. Тобі здається, що це дуже складно, але даремно. Треба зробити перший крок, а той, хто хоче зрозуміти і розібратися, той завжди доб’ється свого', () => this.next());
            yield;

            this.game.displayDialogLine('Ви', '*Ви пошепки запитали про те, що лякало Вас більш за все...*', () => this.next());
            yield;

            this.game.displayDialogLine('Надія', 'Математика? Логіка і аналітика важливі для розробників. Але це не значить, що кожен день ти будеш вирішувати тригонометричні рівняння чи транспонувати матриці. Математичні обчислення беруть на себе різні бібліотеки, що створили за тебе. Так що розслабся, математика - це не фактор відсіювання тру-програмистів', () => this.next());
            yield;

            this.game.displayDialogLine('Надія', 'Щодо комп’ютера, більшість софта не потребує великих потужностей і працює на звичайних ПК, але ти завжди маєш можливість затриматись у кабінеті, де проводяться практичні заняття, і скористатися університетським обладнанням', () => this.next());
            yield;
        ...
        //Выбор: "Подойти к мужчине"
            ...
            this.game.displayDialogLine('Ви', 'Розсудивши, що вибору в Вас не має, Ви розповіли йому про Ваші сумніви у власних здібностях до лякаючи складного фаху ІТ-спеціаліста. Із кожним Вашим словом брови чоловіка іронічно піднімалися вище, а погляд ставав все більш насмішливим. Та Ви витримали і це випробування і не втекли, вперто закінчили свою розповідь', () => this.next());
            yield;

            this.game.displayDialogLine('Чоловік', 'Обрав комп’ютерні науки і злякався якихось чуток. Самому не смішно? Але звернувся ти за адресою, я - завідувач кафедри, до якої ти плануєш поступати. І перше, що я хочу до тебе донести, ІТ-спеціаліст - це не геній, який народжується раз у сторіччя, а звичайна людина, яка готова навчатися і застосовувати свої знання на практиці. Перші кроки завжди важкі. Все, що тобі залишається, визначитись, чи прагнеш ти постійно вдосконалюватись та завантажувати свій мозок роботою, у світі ІТ не зупиняються перед труднощами', () => this.next());
            yield;

            this.game.displayDialogLine('Ви', 'Ви важко зітхнули, йому вдалося Вас присоромити. Та хіба можно було уявити, що він - програмист? Хіба він не повинен бути сутулою, розгубленою людиною з червоними очима і в товстому светрі з плямами від хот-догів і кави? Ох уж ці стереотипи! Ваші сумніви поряд з ним здавалися надуманими та сміховинними', () => this.next());
            yield;
            ...
            this.game.displayDialogLine('Ви', 'З жалем ви зізналися, що математика для вас, подібна трагедії світового рівня.', () => this.next());
            yield;
            ...
            this.game.displayDialogLine('Ви', '*Потрібно віддати належне, цей чоловік умів переконувати. Розмова з ним надала сил позбутися колишніх сумнівів. З відчуттям спокою та впевненості, Ви увійшли до університету*', () => this.next());
            yield;
        }
