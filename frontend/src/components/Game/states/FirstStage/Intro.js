...
export default class IntroState extends Phaser.State {
    * gen() {
        ...
        if (this.answer == 'No'){
            ...
            this.game.displayDialogLine('Голос', 'Це неспроста, напевно, доля! Треба негайно подивитися!', () => this.next());
        }
        yield;

        this.game.setFakeBrowserEnabled(true);
        this.game.displayDialogLine('Голос', 'Комп’ютерні науки? Що ж, спеціаліст з інформаційних технологій - звучить непогано', () =>{
        this.game.displayDialogLine('Голос', 'Ви уявляєте, як сидите в розкішному кріслі власного кабінету, що займає весь останній поверх хмарочосу', () => {
        this.game.displayDialogLine('Голос', 'Ви робите ковток свіжозвареної бразильської кави, що залишив на Вашому столі послужливий особистий помічник, і, не кваплячись, з почуттям власної гідності, декількома надрозумними командами програмуєте космічні машини', () => {
        this.game.displayDialogLine('Голос', 'О, це надзвичайно круто!  Давно забуте почуття наснаги захоплює Вас. З нетерпінням Ви шукаєте на листівці дату - вже завтра!  Треба якнайскоріше зареєструватися та подивитись, де знаходиться університет');
        });
            });
                });
        yield;
