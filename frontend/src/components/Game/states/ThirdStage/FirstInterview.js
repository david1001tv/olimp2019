...
export default class FirstInterviewState extends Phaser.State {
    * gen() {
        ...
        this.game.displayDialogLine('Голос', 'Ви увійшли до кабінету та привітались з його господарем.  Людина, що займала крісло керівника відділу розробки, була стриманою, ввічливою і відчуженою. Ви не могли позбутися незручного відчуття та нервувалися', () => this.next());
        yield;
        ...
        if (this.firstAnswer === 1) {
            this.game.displayDialogLine('Голос', 'Роботодавець дивно на Вас подивився та потім посміхнувся', () => this.next());
        }
        else {
            this.game.displayDialogLine('Голос', 'Роботодавець поважно хитнув головою, мов щось вирішив для себе', () => this.next());
        }
        yield;
        

        this.game.displayDialogLine('Голос', 'Ви раптом збагнули, що завдяки цьому питанню можна було зрозуміти, які завдання цікаві і пріоритетні для Вас, скільки часу Ви відводите на роботу і відпочинок протягом дня...', () => this.next());
        yield;

        this.game.displayDialogLine('Роботодавець', 'Чому ви хочете почати кар\'єру в сфері ІТ?', () => this.next());
        yield;

        this.game.displayDialogLine('Голос', 'Цього разу Ви відразу здогадалися, що мета цього питання - прояснити Вашу мотивацію, бажання і готовність розвиватися в обраній професії. Тож відповіли Ви, обмірковуючи кожне слово, та отримали вже більш зацікавлений погляд', () => this.next());
        yield;

        this.game.displayDialogLine('Роботодавець', 'Що ви зазвичай робите, коли розумієте, що не встигаєте щось зробити, тобто маєте занадто багато завдань?', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Роботодавець', 'За відповіддю, а саме за способом вирішення цієї проблеми, я можу багато про що судити. Якщо людина бере на себе всі завдання, то вона скоріш за все не командний гравець, такий собі герой-одинак', () => this.next());
        yield;

        this.game.displayDialogLine('Роботодавець', 'Якщо людина звернеться до менеджера з питання пріоритетності завдань і додаткових ресурсів, то вона раціональна, проактивна і, наймовірніше, буде рости', () => this.next());
        yield;

        this.game.displayDialogLine('Голос', 'Ви кивнули у відповідь, як кожна розумна людина. Співбесіда підходила до кінця, і Вас запитали про Ваші досягнення', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Голос', 'За результатами співбесіди Вам запропонували декілька варіантів працевлаштування, кожний зі своїми вимогами, деякі з котрих залежали від обраних Вами дисциплін під час навчання в університеті. Тож деякі вакансії були Вам недоступні через нестачу знань', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Голос', 'Перша співбесіда залишила незабутні враження, але Ви ще не були впевнені, що саме ця посада потрібна Вам, тому Ви вирушили до наступного кабінету', () => this.next());
        yield;
