...
export default class ThirdInterviewState extends Phaser.State {
    * gen() {
        ...
        this.game.displayDialogLine('Голос', 'З цим роботодавцем у Вас була попередня співбесіда за телефоном. Це було добрим знаком - Ваше резюме пройшло первинний відбір. Ви отримали запрошення пройти очну співбесіду та вже не так сильно хвилювалися', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Голос', 'Ви проявили самовладання і дотримувалися правил пристойності - ввічливість, доброзичливість і повага до чужої думки, хоча й іноді бажання перебити і розкритикувати конкурента, який говорив дурниці, затьмарювало втяму', () => this.next());
        yield;

        this.game.displayDialogLine('Голос', 'Уважно вислухав кожного конкурента, Ви зфіксували слабкі і сильні місця в відповідях, і будували свої відповіді з урахуванням поміченого', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Голос', 'Також Вас запитали, що як Ви плануєте своє майбутнє. Адміністрування систем Windows - це відмінний шлюз в безліч різних професій. Ви з пристрастю відповіли, що мрієте піти в управління, щоб стати наступним CIO', () => this.next());
        yield;

        this.game.displayDialogLine('Голос', 'Тож роботодавець відмітив, що Ви будете намагатися видати кращий результат, щоб просуватися вгору по сходах. Вам запропонували кілька вакансій', () => this.next());
        yield;
        ...
        this.game.displayDialogLine('Голос', 'Як же співбесіди виснажують морально! Але кабінет останнього роботодавця Ви покидали з легким серцем, адже не розгубились і показали себе з кращого боку', () => this.next());
        yield;
